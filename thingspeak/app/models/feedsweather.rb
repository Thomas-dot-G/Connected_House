...
class Feedsweather < ActiveRecord::Base
  extend FeedHelper
  belongs_to :channel
  ...
  def self.delete_in_batches(channel_id)
    ...
    while channel.feeds.count > 0
      ...
      sql = "DELETE FROM feedsweathers WHERE channel_id=#{channel_id} LIMIT 1000"
  ...
  def self.select_options(channel, params)
    ...
    only += [:temperature] unless  (params[:field_id] and !params[:field_id].index('1') )
    only += [:pressure]  unless  (params[:field_id] and !params[:field_id].index('2'))
    only += [:humidity]  unless (params[:field_id] and !params[:field_id].index('3'))
    only += [:pluviometry] unless  (params[:field_id] and !params[:field_id].index('4'))
    only += [:luminosity] unless  (params[:field_id] and !params[:field_id].index('5'))
    only += [:wms] unless (params[:field_id] and !params[:field_id].index('6'))
    only += [:wdir] unless  (params[:field_id] and !params[:field_id].index('7'))
    only += [:was] unless  (params[:field_id] and !params[:field_id].index('8'))

class FeedsweatherFactory < ApplicationController
  include FeedHelper
  ...
  def get_feed_data_for_date(date)
    ...
    feeds = Feedsweather.where(:channel_id => @channel.id, :created_at => date..(date + 1.day))
    ...
    feed = Feedsweather.new(:created_at => date) if feed.nil?
  ...
  def get_feeds
    ...
    if @options[:start_entry_id].present? || @options[:end_entry_id].present?
      @feeds = Feedsweather.from("feedsweathers FORCE INDEX (index_feeds_on_channel_id_and_entry_id)")
  ...
  def get_daily_feeds
    ...
    @daily_feeds = Feedsweather.normalize_feeds(@daily_feeds) if @daily_feeds.present?
